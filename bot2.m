...
function [x_k, P_k] = filtro_kalman(z_k, x_k, P_k, Q, R)
    I = eye(2);
    A = eye(2);
    H = eye(2);
...
for passo = 1:500
    ...
    z_k = detectar_wonderboy_rgb(tela);
    if all(z_k > 0)
        [x_k, P_k] = filtro_kalman(z_k, x_k, P_k, Q, R);
    end
    centro = x_k;
...
if pulando > 0
    ...
    text(z_k(1), size(tela,1)-z_k(2), 'Z', 'Color', 'r', 'FontSize', 12); % Raw
    text(x_k(1), size(tela,1)-x_k(2), 'K', 'Color', 'g', 'FontSize', 14); % Kalman
